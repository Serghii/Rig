...
class GSStopMinerCmd : AttributesReaderBaseCmd
{
    public GSStopMinerCmd(IGSheet gSheet) : base(gSheet) { }

    public override string GetAttributesType => GShSettings.stopMiner;
    public override void Execute(IList<object> line)
    {
        ...
        if (TryParse(CpuLoadMin, CpuLoadMax, SensorsType.CpuLoad, out newAlarm))
            gSheet.Data.StopSettingsList.AddOrReplase(newAlarm);
